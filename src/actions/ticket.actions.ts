...
export async function createTicket(prevState: {success: boolean;
...
export async function getTickets(){
  try{
    ...
    logEvent('Fetched ticket list', 'ticket',{count:
        tickets.length}, 'info');
...
export async function closeTicket(prevState: {success:
  boolean; message: string}, formData: FormData): Promise<
  {success: boolean, message: string}> {
    ...
    if(!ticket || ticket.userId !== user.id){
      ...
      return {success: false,message: 'You are not authorized to close the ticket'}
    }
